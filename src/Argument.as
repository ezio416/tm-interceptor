class MethodArgument {
    ...
    string[]@ GenerateLines() final {
        ...
        if (type.EndsWith("@")) {  // nod
            return {
                'auto ' + name + ' = cast<' + (type.SubStr(0, type.Length - 1)) + '>(stack.CurrentNod(' + stackIndex + '));',
                'print("' + type + ' ' + name + ': " + (' + name + ' !is null ? "valid" : "null"));'
            };
        }

        if (type.Contains("::E")) {  // enum
            return {
        ...
        if (type == "int2") {  // .CurrentInt2() doesn't exist yet
            return {
        ...
        if (type == "vec2") {  // .CurrentVec2() doesn't exist yet
            return {
